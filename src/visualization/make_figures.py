...
def figures():
 ...
 fig,ax=plt.subplots(figsize=(12,2.4)); ax.axis('off'); ax.text(.5,.5,"Generate -> Validate -> MySQL 8 -> SQL Analytics -> Export -> Visualize",ha='center',va='center',fontsize=16,weight='bold'); save("01_project_pipeline.png")
 ...
 f=pd.read_csv(T/"conversion_funnel.csv").iloc[0]; cols=["view_users","cart_users","purchase_users","paid_users"]; plt.figure(figsize=(8,4.5)); plt.bar(["View","Cart","Purchase","Payment"],[f[c] for c in cols],color="#2878B5"); plt.title("User Conversion Funnel"); save("03_conversion_funnel.png")
